    ...
    if (empty($error)) {
        if ($_FILES["avatar"]["error"] === 4) {
            ...
            if (!in_array($image_extension, $valid_image_extension)) {
                ...
                $update = "UPDATE users INNER JOIN departmenttable ON users.dept_id = departmenttable.dept_id SET users.dept_id = '$dept_id', users.fullname = '$fullname', users.username = '$username', avatar = '$new_image_name' WHERE users.id = '{$_SESSION[AUTH_ID]}' ";
