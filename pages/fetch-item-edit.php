<?php
    if(isset($_POST["action"])){
        ...
        if($_POST["action"] == "category"){       
            $query = $DB->query (" SELECT item_name FROM itemtable WHERE cat_id = '".$_POST["query"]."' GROUP BY item_name ");
        ...
        if($_POST["action"] == "item_name"){
            $query = $DB->query (" SELECT unit FROM itemtable WHERE item_name = '".$_POST["query"]."'");
        ...
        if($_POST["action"] == "unit"){
            $query = $DB->query (" SELECT cost FROM itemtable WHERE unit = '".$_POST["query"]."'");
